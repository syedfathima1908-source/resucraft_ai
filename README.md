...
1. **Opaque ATS Filtering**: Candidates lack clear, explainable feedback on why their resume was rejected or how keyword matching and similarity scores are calculated.
2. **Generic Resume Formatting**: Standard resumes fail to highlight critical job-description-specific skills or use high-impact action verbs.
3. **Interview Anxiety & Lack of Practice**: Traditional mock interviews are expensive, hard to schedule, and rarely tailored to a candidate's actual projects or technical stack.
...
```mermaid
graph TD
    User([User / Browser]) <-->|React 19 + Vite + Tailwind| Frontend[Frontend Client App]
    Frontend <-->|Supabase JS SDK| Supabase[(Supabase DB & Auth)]
    Frontend <-->|HTTP REST / JSON| Backend[Flask Backend API]
    Backend <-->|PyPDF / Docx / NLTK / Scikit-Learn| ML[ML & ATS Engine]
    Backend <-->|google-genai SDK| Gemini[Google Gemini 2.5 Flash]
```
...
sequenceDiagram
    ...
    Candidate->>FE: Upload Resume (PDF/DOCX) + Job Description
    FE->>BE: POST /api/analyze
    BE->>BE: Extract Text & Run TF-IDF / Skill Taxonomy ML Engine
    BE-->>FE: Return ATS Match Score & Skill Breakdown
    FE->>DB: Save Analysis Record to analysis_history

    Candidate->>FE: View AI Optimizer Page
    FE->>BE: POST /api/optimize-full
    BE->>AI: Generate Bullet Rewrites & Recommendations
    AI-->>BE: Return JSON Optimization Data
    BE-->>FE: Display Recommendations & Bullet Changes

    Candidate->>FE: Start Voice AI Mock Interview
    FE->>BE: POST /api/interview/generate-questions
    BE->>BE: Extract Resume Projects & Skills
    BE->>AI: Generate Grounded Questions
    AI-->>BE: Return Questions
    BE-->>FE: Deliver Interview Questions

    Candidate->>FE: Speak Answer (Web Speech API)
    FE->>BE: POST /api/interview/evaluate-answer
    BE->>AI: Evaluate Answer Qualitatively
    AI-->>BE: Return Qualitative Feedback
    BE-->>FE: Display Question Feedback

    Candidate->>FE: Finalize Interview
    FE->>BE: POST /api/interview/finalize
    BE-->>FE: Return Final Performance Report Card
    FE->>DB: Save Interview Record to interview_history
```
...
- **Supabase Account**: Created project on [Supabase](https://supabase.com/)
...
3. Ensure **Email Auth** is enabled under **Authentication Settings**.
...
- **GitHub**: [github.com/SyedFathima](https://github.com/SyedFathima)
